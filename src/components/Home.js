    ...
    style={{ minHeight: '100vh', minWidth: '100vw' }}>
      <Grid item xs={12} sm={12} md={12} lg={12}>
        <ButtonGroup variant="text" size='large' className={classes.buttonGroup} >
          <Button onClick={changeLastRace} color={showLastRace ? 'primary' : 'secondary'}>Last Race</Button>
          <Button onClick={changeDriverTable} color={showDriverTable ? 'primary' : 'secondary'}>Driver Standings</Button>
          <Button onClick={changeConstructorTable} color={showConstructorTable ? 'primary' : 'secondary'}>Constructor Standings</Button>
        </ButtonGroup>
      </Grid>
        {lastRaceData && showLastRace &&
        <Grid item xs={12} sm={12} md={12} lg={12}>
          <TableContainer className={classes.paper} component={Paper}>
          <Typography component={'span'} variant='h5' >{`${lastRaceData[lastRaceData.length -1].Circuit.circuitName} Round: ${lastRaceData[lastRaceData.length -1].round}`}</Typography>
      ...
      {lastRaceData && showDriverTable &&
        <Grid  item xs={12} sm={12} md={12} lg={12}>
          <TableContainer className={classes.paper} component={Paper}>
          <Typography component={'span'} variant='h5' >Current Championship Standings</Typography>
          <Table className={classes.table}>
            <TableHead>
              <TableRow>
                <StyledTableCell>Pos</StyledTableCell>
      ...
      {lastRaceData && showConstructorTable &&
        <Grid  item xs={12} sm={12} md={12} lg={12}>
          <TableContainer className={classes.paper} component={Paper}>
          <Typography component={'span'} variant='h5' >Current Constructor Standings</Typography>
          <Table className={classes.constructorTable}>
            ...
            <TableBody>
              {constructorData && constructorData.map((team) => {
